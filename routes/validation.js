...
router.get('/', isAuthenticated, role('finance'), (req, res) => {
...
router.post('/po/:id/approve', isAuthenticated, role('finance'), (req, res) => {
  ValidationService.approvePurchaseOrder(req.params.id, req.session.userId);
  res.redirect('/validation?success=PO berhasil divalidasi');
});

router.post('/po/:id/reject', isAuthenticated, role('finance'), (req, res) => {
  ValidationService.rejectPurchaseOrder(req.params.id, req.session.userId, req.body.catatan || '');
  res.redirect('/validation?success=PO ditolak');
});

router.post('/cost/:id/approve', isAuthenticated, role('finance'), (req, res) => {
  ValidationService.approveProductionCost(req.params.id, req.session.userId);
  res.redirect('/validation?success=Biaya produksi berhasil divalidasi');
});

router.post('/cost/:id/reject', isAuthenticated, role('finance'), (req, res) => {
  ValidationService.rejectProductionCost(req.params.id, req.session.userId, req.body.catatan || '');
  res.redirect('/validation?success=Biaya produksi ditolak');
});

router.post('/import/:id/approve', isAuthenticated, role('finance'), (req, res) => {
  ValidationService.approvePurchaseImport(req.params.id, req.session.userId);
  res.redirect('/validation?success=Pembelian import berhasil divalidasi');
});

router.post('/import/:id/reject', isAuthenticated, role('finance'), (req, res) => {
  ValidationService.rejectPurchaseImport(req.params.id, req.session.userId, req.body.catatan || '');
